	...
	<div class="row">
		<div class="col-md-6">
			...
			<div class="form-group">
				<label for="faculty_id" class="control-label">Association MemberID</label>
				<input type="text" class="form-control form-control-sm" name="faculty_id" id="faculty_id" value="<?php echo isset($faculty_id) ? $faculty_id : '' ?>" required>
			</div>
			<div class="form-group">
		...
		$('#faculty-frm').submit(function(e){
			...
			$.ajax({
				...
				if(resp == 1){
					...
					var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Faculty ID already exists.</div>"
					_frm.prepend(_msg)
					_frm.find('input#faculty_id').addClass('is-invalid')
					$('[name="faculty_id"]').focus()
				}else{
